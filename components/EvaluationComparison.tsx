...
import { RUBRIC_DIMENSIONS, HARM_SCALE, YES_NO_UNSURE_OPTIONS, DISPARITY_CRITERIA } from '../constants';
    ...
    const renderSingleResponseScores = (human: LanguageSpecificRubricScores, llm: LlmRubricScores) => (
        <div className="bg-background p-4 rounded-lg border border-border/70 space-y-2">
            {RUBRIC_DIMENSIONS.map(dim => {
                ...
                const humanDisplay = dim.isSlider ? `${getHarmScaleLabel(humanVal as number)} (${humanVal})` : getCategoricalOptionLabel(dim.key, humanVal as string);
                const llmDisplay = dim.isSlider ? `${getHarmScaleLabel(llmVal as number)} (${llmVal})` : getCategoricalOptionLabel(dim.key, llmVal as string);
